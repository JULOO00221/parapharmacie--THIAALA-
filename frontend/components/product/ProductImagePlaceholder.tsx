 ...
 * the default. A soft brand-toned tile with a simple droplet icon reads
 * as intentional, not as a broken/missing image.
 */
export function ProductImagePlaceholder({ className }: { className?: string }) {
  return (
    <div
      className={cn('flex items-center justify-center bg-brand-50', className)}
      role="img"
      aria-label="Photo du produit non disponible"
    >
      <svg viewBox="0 0 40 40" className="h-1/3 w-1/3 text-brand-200" fill="none" aria-hidden="true">
        <path
